...
function init() {
    const saved = localStorage.getItem("aus_wh_v4");
...
function save() { localStorage.setItem("aus_wh_v4", JSON.stringify(state)); }
...
function updateUI() {
    ...
    accList.innerHTML = state.accounts.map(a => `
        <div style="display:flex; justify-content:space-between; margin-bottom:5px;">
            ...
            <span style="font-size:0.75rem; font-weight:bold;">${a.curr} ${a.balance.toLocaleString()}</span>
        </div>
    `).join('');
    ...
    const totalTWD = (totalAUD * r.AUD_TWD);
    const totalUSD = (totalAUD * r.AUD_USD);

    document.getElementById("eval-aud").innerText = `$ ${totalAUD.toFixed(2)}`;
    document.getElementById("eval-twd").innerText = `$ ${Math.round(totalTWD).toLocaleString()}`;
    document.getElementById("eval-usd").innerText = `$ ${totalUSD.toFixed(2)}`;
    ...
    transList.innerHTML = state.transactions.slice(0, 5).map(t => `
        <div class="transaction-item">
            ...
            <span class="${t.type}" style="font-size:0.8rem; font-weight:bold;">${t.type==='income'?'+':'-'}${t.amount}</span>
        </div>
    `).join('');
}
...
function addTransaction() {
    ...
    state.transactions.unshift({ id: Date.now(), type, amount, desc, accId, accName: acc.name, date: new Date().toLocaleDateString() });
...
function renderAccountManage() {
    ...
    list.innerHTML = state.accounts.map(a => `<div style="display:flex; justify-content:space-between; margin-bottom:5px;"><span class="custom-text">${a.name} (${a.curr})</span><button onclick="deleteAcc('${a.id}')" style="color:red; border:none; background:none;">刪除</button></div>`).join('');
}
